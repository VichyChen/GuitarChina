...
- (NSString *)getGCThreadDetailModelHtml {
    ...
    for (GCThreadDetailPostModel *item in self.postlist) {
        if ([item.number isEqualToString:@"1"] && self.optionlist.count > 0) {
            ...
            [htmlCellString appendFormat:htmlCell, GCNETWORKAPI_SMALLAVTARIMAGE_URL(item.authorid),item.author, item.dateline, [item.number isEqualToString:@"1"] ? @"楼主" : [NSString stringWithFormat:@"%@楼", item.number], tableString, item.message];
        } else {
            [htmlCellString appendFormat:htmlCell, GCNETWORKAPI_SMALLAVTARIMAGE_URL(item.authorid), item.author, item.dateline, [item.number isEqualToString:@"1"] ? @"楼主" : [NSString stringWithFormat:@"%@楼", item.number], @"", item.message];
        }
    }
