...
- (void)setSelectPickerAssets:(NSArray *)selectPickerAssets{
    ...
    for (ZLPhotoAssets *assets in selectPickerAssets) {
        if ([assets isKindOfClass:[ZLPhotoAssets class]] || [assets isKindOfClass:[UIImage class]] || [assets isKindOfClass:[NSString class]]) {
...
//    self.selectAssets = [NSMutableArray arrayWithArray:[[NSSet setWithArray:self.selectAssets] allObjects]];
    ...
    if (self.selectPickerAssets.count || deleteAssets) {
        ...
        for (NSInteger i = 0; i < self.selectAssets.count; i++) {
            ...
            if ([photoAsset isKindOfClass:[UIImage class]] || [photoAsset isKindOfClass:[NSString class]]) {
                continue;
            }
...
- (UICollectionViewCell *) collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.selectAssets.count > indexPath.item) {
        ...
        } else if ([self.selectAssets[indexPath.item] isKindOfClass:[NSString class]]){
            [imageView sd_setImageWithURL:[NSURL URLWithString:self.selectAssets[indexPath.item]]];
        }
    }
...
-  (ZLPhotoPickerBrowserPhoto *)photoBrowser:(ZLPhotoPickerBrowserViewController *)pickerBrowser photoAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (self.selectAssets.count && self.selectAssets.count - 1 >= indexPath.item) {
        ...
        if ([asset isKindOfClass:[ZLPhotoAssets class]]) {
            ...
            photo.photoURL = [NSURL URLWithString: (NSString *)asset];
        }
    }
